...
- [ ] `p2` - **ID**: `cpt-cf-oagw-flow-tenant-share-upstream`
...
**Steps**:
1. [ ] - `p2` - Platform operator creates upstream with sharing configuration via Management API - `inst-share-1`
2. [ ] - `p2` - API: POST /api/oagw/v1/upstreams (body includes sharing fields per `cpt-cf-oagw-component-model`) - `inst-share-2`
3. [ ] - `p2` - Validate sharing mode for each field is one of `private`, `inherit`, `enforce` - `inst-share-3`
...
- [ ] `p2` - **ID**: `cpt-cf-oagw-flow-tenant-bind-inherited`
...
**Steps**:
1. [ ] - `p2` - Tenant admin submits upstream creation request that resolves to an existing ancestor upstream alias - `inst-bind-1`
2. [ ] - `p2` - API: POST /api/oagw/v1/upstreams (alias matches ancestor upstream) - `inst-bind-2`
3. [ ] - `p2` - Resolve ancestor upstream by walking hierarchy via `cpt-cf-oagw-algo-tenant-alias-shadow` - `inst-bind-3`
4. [ ] - `p2` - Check `oagw:upstream:bind` permission via `cpt-cf-oagw-algo-tenant-permission-check` - `inst-bind-4`
5. [ ] - `p2` - **IF** permission denied - `inst-bind-5`
   1. [ ] - `p2` - **RETURN** 403 Forbidden with missing permission details - `inst-bind-5a`
6. [ ] - `p2` - **FOR EACH** overridden field in request - `inst-bind-6`
   1. [ ] - `p2` - **IF** ancestor field sharing is `enforce` - `inst-bind-6a`
      1. [ ] - `p2` - **RETURN** 400 ValidationError: field cannot be overridden (sharing: enforce) - `inst-bind-6a1`
   2. [ ] - `p2` - **IF** ancestor field sharing is `private` - `inst-bind-6b`
      1. [ ] - `p2` - **RETURN** 400 ValidationError: field not visible (sharing: private) - `inst-bind-6b1`
   3. [ ] - `p2` - **IF** field is auth and override requested - `inst-bind-6c`
      1. [ ] - `p2` - Check `oagw:upstream:override_auth` permission - `inst-bind-6c1`
      2. [ ] - `p2` - **IF** auth override has `secret_ref` - `inst-bind-6c2`
         1. [ ] - `p2` - Validate `secret_ref` accessibility via `cred_store` for descendant tenant - `inst-bind-6c2a`
         2. [ ] - `p2` - **IF** secret not accessible - `inst-bind-6c2b`
            1. [ ] - `p2` - **RETURN** 400 ValidationError: secret_ref not accessible to tenant - `inst-bind-6c2b1`
   4. [ ] - `p2` - **IF** field is rate_limit and override requested - `inst-bind-6d`
      1. [ ] - `p2` - Check `oagw:upstream:override_rate` permission - `inst-bind-6d1`
   5. [ ] - `p2` - **IF** field is plugins and additions requested - `inst-bind-6e`
      1. [ ] - `p2` - Check `oagw:upstream:add_plugins` permission - `inst-bind-6e1`
7. [ ] - `p2` - **IF** `cred_store` is unavailable during secret_ref validation (timeout or connection error) - `inst-bind-7`
   1. [ ] - `p2` - **RETURN** 503 ServiceUnavailable: credential validation unavailable (fail-closed) - `inst-bind-7a`
8. [ ] - `p2` - **IF** request includes tags - `inst-bind-8`
   1. [ ] - `p2` - Treat tags as tenant-local additions; do not mutate ancestor tags - `inst-bind-8a`
9. [ ] - `p2` - DB: BEGIN transaction - `inst-bind-9`
10. [ ] - `p2` - DB: INSERT oagw_upstream with descendant tenant_id, storing local overrides - `inst-bind-10`
11. [ ] - `p2` - **IF** `(tenant_id, alias)` uniqueness constraint fails - `inst-bind-11`
    1. [ ] - `p2` - DB: ROLLBACK - `inst-bind-11a`
    2. [ ] - `p2` - **RETURN** 409 Conflict with alias collision details - `inst-bind-11b`
12. [ ] - `p2` - DB: COMMIT - `inst-bind-12`
13. [ ] - `p2` - **RETURN** created upstream binding with effective configuration - `inst-bind-13`

### Alias Resolution with Shadowing

- [ ] `p2` - **ID**: `cpt-cf-oagw-flow-tenant-alias-resolve`
...
**Steps**:
1. [ ] - `p2` - System receives proxy request with alias in path: `{METHOD} /api/oagw/v1/proxy/{alias}/{path}` - `inst-resolve-1`
2. [ ] - `p2` - Extract requesting tenant_id from SecurityContext - `inst-resolve-2`
3. [ ] - `p2` - Resolve upstream via `cpt-cf-oagw-algo-tenant-alias-shadow` - `inst-resolve-3`
4. [ ] - `p2` - **IF** no upstream found in hierarchy - `inst-resolve-4`
   1. [ ] - `p2` - **RETURN** 404 RouteNotFound - `inst-resolve-4a`
5. [ ] - `p2` - **IF** resolved upstream is disabled and no enabled ancestor exists - `inst-resolve-5`
   1. [ ] - `p2` - **RETURN** 503 LinkUnavailable - `inst-resolve-5a`
6. [ ] - `p2` - Compute effective configuration via `cpt-cf-oagw-algo-tenant-config-merge` - `inst-resolve-6`
7. [ ] - `p2` - **RETURN** effective upstream configuration for proxy execution - `inst-resolve-7`
...
**Steps**:
1. [ ] - `p2` - System computes effective configuration after alias resolution - `inst-enforce-1`
2. [ ] - `p2` - Collect all ancestor upstreams in hierarchy with `sharing: enforce` on any field - `inst-enforce-2`
3. [ ] - `p2` - **FOR EACH** enforced ancestor upstream - `inst-enforce-3`
   1. [ ] - `p2` - **IF** field is rate_limit with `sharing: enforce` - `inst-enforce-3a`
      1. [ ] - `p2` - Apply `min(enforced_rate, effective_rate)` per `cpt-cf-oagw-algo-tenant-effective-rate` - `inst-enforce-3a1`
   2. [ ] - `p2` - **IF** field is auth with `sharing: enforce` - `inst-enforce-3b`
      1. [ ] - `p2` - Override effective auth with ancestor's enforced auth configuration - `inst-enforce-3b1`
   3. [ ] - `p2` - **IF** field is plugins with `sharing: enforce` - `inst-enforce-3c`
      1. [ ] - `p2` - Ensure enforced plugins remain in chain; descendant cannot remove them - `inst-enforce-3c1`
...
4. [ ] - `p2` - **RETURN** final effective configuration with all enforced constraints applied - `inst-enforce-4`
...
- [ ] `p2` - **ID**: `cpt-cf-oagw-algo-tenant-alias-shadow`
...
2. [ ] - `p2` - **FOR EACH** tenant_id in chain (self first) - `inst-shadow-2`
   ...
   2. [ ] - `p2` - **IF** upstream found AND (tenant_id == requesting_tenant OR any per-field sharing flag — `auth_sharing`, `rate_limit_sharing`, `plugins_sharing`, `cors_sharing` — is != `private`) - `inst-shadow-2b`
      ...
      2. [ ] - `p2` - **ELSE** (upstream disabled) - `inst-shadow-2b2`
         1. [ ] - `p2` - Record as disabled match; continue walking for enabled ancestor - `inst-shadow-2b2a`
3. [ ] - `p2` - **IF** no match found in entire chain - `inst-shadow-3`
   1. [ ] - `p2` - **RETURN** not-found error - `inst-shadow-3a`
4. [ ] - `p2` - **IF** only disabled matches found - `inst-shadow-4`
   1. [ ] - `p2` - **RETURN** link-unavailable error - `inst-shadow-4a`

### Permission Validation for Override

- [ ] `p2` - **ID**: `cpt-cf-oagw-algo-tenant-permission-check`
...
**Steps**:
1. [ ] - `p2` - Extract permissions from SecurityContext bearer token via `modkit-auth` - `inst-perm-1`
2. [ ] - `p2` - **IF** required permission is `oagw:upstream:bind` - `inst-perm-2`
   1. [ ] - `p2` - Check token has `oagw:upstream:bind` for target upstream's tenant scope - `inst-perm-2a`
...
6. [ ] - `p2` - **IF** permission check fails - `inst-perm-6`
   1. [ ] - `p2` - **RETURN** denied with missing permission and sharing mode reason - `inst-perm-6a`
7. [ ] - `p2` - **RETURN** allowed - `inst-perm-7`

### Effective Rate Limit Computation

- [ ] `p2` - **ID**: `cpt-cf-oagw-algo-tenant-effective-rate`
...
4. [ ] - `p2` - **RETURN** candidate as effective rate limit - `inst-rate-4`
...
The system **MUST** merge configurations across tenant hierarchy with the following per-field strategies: auth override if `inherit`; rate limits `min(ancestor.enforced, descendant)` — stricter always wins; plugins concatenation (ancestor + descendant); tags union with add-only semantics; CORS union origins if `inherit`, forced if `enforce`. The system **MUST** apply the 3-layer merge priority (upstream base < route < tenant) per `cpt-cf-oagw-fr-config-layering`. Absent/null fields **MUST** inherit from the previous level (absent rate_limit = unbounded). Enforced ancestor constraints **MUST** never be bypassed.
...
- [ ] `p2` - **ID**: `cpt-cf-oagw-dod-tenant-alias-shadowing`
...
- [ ] `p2` - **ID**: `cpt-cf-oagw-dod-tenant-permission-control`
...
- [ ] `p2` - **ID**: `cpt-cf-oagw-dod-tenant-secret-access`
...
- [ ] Sharing modes (`private`, `inherit`, `enforce`) can be set per field on upstream create and update
- [ ] Default sharing mode is `private` when not specified
- [ ] Descendant with `oagw:upstream:bind` permission can create binding to ancestor's inherited upstream
- [ ] Descendant without `oagw:upstream:bind` permission receives 403 Forbidden
- [ ] Auth override with `sharing: inherit` succeeds when descendant has `oagw:upstream:override_auth`
- [ ] Auth override with `sharing: enforce` is rejected with 400 ValidationError
- [ ] Rate limit effective value equals `min(all_ancestor_enforced_rates, descendant_rate, route_rate)`
- [ ] Plugin merge produces concatenated chain: `ancestor.plugins + descendant.plugins`
- [ ] Enforced ancestor plugins cannot be removed by descendant
- [ ] Tag merge produces union; descendants can add but not remove inherited tags
- [ ] CORS merge unions origins for `inherit`; uses ancestor config for `enforce`
- [ ] Alias resolution walks tenant hierarchy from descendant to root; closest enabled match wins
- [ ] Alias shadowing preserves all enforced ancestor constraints on the shadowed upstream
- [ ] Disabled upstream is skipped during resolution, falling through to next ancestor
- [ ] `secret_ref` in auth override is validated against `cred_store` for descendant tenant accessibility
- [ ] Inaccessible `secret_ref` is rejected with 400 ValidationError
- [ ] No secret material is stored or logged by OAGW
- [ ] All DB operations use secure ORM with tenant scoping per `cpt-cf-oagw-principle-tenant-scope`
- [ ] Route-level sharing modes (`private`, `inherit`, `enforce`) can be set per field on route create and update
- [ ] 3-layer merge applies: upstream base < route overrides < tenant hierarchy overrides
- [ ] Absent/null fields inherit from the previous level; absent rate_limit is treated as unbounded
- [ ] `cred_store` unavailability during secret_ref validation results in 503 ServiceUnavailable (fail-closed)
